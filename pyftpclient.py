        ...
        with open(local_filename, 'w+b') as f:
            ...
            def monitor():
                if not self.waiting:
                    i = f.tell()
                    if self.ptr < i:
                        logging.debug("%s - %d kbytes -  %0.1f Kb/s\t[%2.2f %%]" %
                                      (local_filename, i/1024, (i - self.ptr) / (1024 * self.monitor_interval),
                                       float(i) / self.__dst_filesize * 100))
                        self.ptr = i
                    else:
                        ftp.close()
            ...
            ftp.voidcmd('TYPE I')
            try:
                self.__dst_filesize = ftp.size(dst_filename)
            except:
                return None

            mon = monitor()
            while self.__dst_filesize > f.tell():
                try:
                    connect()
                    self.waiting = False
